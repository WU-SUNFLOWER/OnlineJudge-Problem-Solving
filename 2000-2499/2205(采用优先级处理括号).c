...
int solve(char* exp, char* exp_prefix, int start, int end, int start_prefix) {
    ...
    while (index < end) {
        int curIndex = index;
        char ch = exp[index++];
        // 如果当前字符是左括号，强制入栈，为后面输出子表达式的处理结果做准备
        if (ch == '(') {
            pushToStack(stack_operation, &ch);
        }
        // 如果当前字符是右括号，则说明
        // 最靠近其的被左右括号包裹的子表达式已经计算完毕。
        // 此时需要刷新堆栈，将结果存入exp_prefix
        // 并且同时需要把与当前相匹配的右括号一并出栈
        else if (ch == ')') {
            while (popFromStack(stack_operation, &ch)) {
                if (ch == '(') break;
                exp_prefix[index_prefix++] = ch;
            }
        }
...
int main() {
    ...
    // 初始化优先级表
    // 左括号的优先级最低，这是为了在循环批量出栈的时候，能够在遇到左括号的时候停下来
    priority['('] = 0;  
    priority['+'] = 1;
    priority['-'] = 1;
    priority['*'] = 2;
