#include <stdio.h>
#include <string.h>
#include <stdlib.h>

typedef struct node {
   int val;
   struct node* next;
   struct node* pre;
} *Node;

Node createNode(Node pre, int val) {
    Node n = (Node)malloc(sizeof(struct node));
    n->val = val;
    n->pre = pre;
    n->next = NULL;
    return n;
}

void printList(Node head) {
    Node curNode = head->next;
    while (curNode != NULL) {
        printf("%d", curNode->val);
        curNode = curNode->next;
    }
    putchar('\n');
}

void update(Node head) {
    Node lastNode = head;
    Node beginNode;
    Node curNode = lastNode->next;
    int cnt = -1;
    int curVal = -1;
    while (curNode != NULL) {
        // 第一种情况，计数器还在持续计数
        if (curNode->val == curVal) {
            cnt++;
        }
        // 第二种情况，碰到了新数字
        else {
            // 如果计数器已经满3了，则执行清除操作
            if (cnt >= 3) {
                Node lastNode_beCleared;
                Node curNode_beCleared = beginNode->next;
                while (curNode_beCleared != curNode) {
                    lastNode_beCleared = curNode_beCleared;
                    curNode_beCleared = lastNode_beCleared->next;
                    free(lastNode_beCleared);
                }
                beginNode->next = curNode;
                curNode->pre = beginNode;
            }
            // 如果计数器没满3，别忘了重新定位beginNode
            else {
                beginNode = lastNode;
            }
            curVal = curNode->val;
            cnt = 1;
        }
        lastNode = curNode;
        curNode = curNode->next;
    }
    // 特判需要清除表尾元素的情况
    if (cnt >= 3) {
        Node lastNode_beCleared;
        Node curNode_beCleared = beginNode->next;
        while (curNode_beCleared != NULL) {
            lastNode_beCleared = curNode_beCleared;
            curNode_beCleared = lastNode_beCleared->next;
            free(lastNode_beCleared);
        }
        beginNode->next = NULL;
    }
}

void update_partial(Node head, int pos) {
    // 定位到目标节点
    int cnt = 0;
    Node startNode;
    Node lastNode = head;
    Node curNode = lastNode->next;
    while (cnt < pos && curNode != NULL) {
        lastNode = curNode;
        curNode = curNode->next;
        cnt++;
    }
    startNode = curNode;
    // 向左扫描
    Node preNode;
    Node nextNode;
    curNode = startNode->pre;
    while (curNode != head && curNode->val == startNode->val) {
        preNode = curNode->pre;
        // 删除当前节点
        nextNode = curNode->next;
        preNode->next = nextNode;
        if (nextNode) nextNode->pre = preNode;
        free(curNode);
        // 更新curNode
        curNode = preNode;
    }
    // 向右扫描
    curNode = startNode->next;
    while (curNode != NULL && curNode->val == startNode->val) {
        nextNode = curNode->next;
        // 删除当前节点
        preNode = curNode->pre;
        preNode->next = nextNode;
        if (nextNode) nextNode->pre = preNode;
        free(curNode);
        // 更新curNode
        curNode = nextNode;
    }
    // 删除startNode本身
    nextNode = startNode->next;
    preNode = startNode->pre;
    preNode->next = nextNode;
    if (nextNode) nextNode->pre = preNode;
    free(startNode);
}

void insert(Node head, int pos, int val) {
    int cnt = 0;
    Node lastNode = head;
    Node curNode = lastNode->next;
    while (cnt < pos && curNode != NULL) {
        lastNode = curNode;
        curNode = curNode->next;
        cnt++;
    }
    Node newNode = createNode(lastNode, val);
    lastNode->next = newNode;
    newNode->next = curNode;
    if (curNode) curNode->pre = newNode;
}

void change(Node head, int pos) {
    int cnt = 0;
    Node lastNode = head;
    Node curNode = lastNode->next;
    while (cnt < pos - 1 && curNode != NULL) {
        lastNode = curNode;
        curNode = curNode->next;
        cnt++;
    }
    curNode->val = lastNode->val;
}


int parseInt(char str[]) {
    int num = 0;
    char ch;
    for (int i = 0; ch = str[i]; i++) {
        num = num * 10 + (ch - '0');
    }
    return num;
}

int main() {
    char str[100];
    Node headNode = createNode(NULL, -1);
    Node lastNode = headNode;
    Node curNode;
    scanf("%s", str);
    for (int i = 0; str[i]; i++) {
        curNode = createNode(lastNode, str[i] - '0');
        lastNode->next = curNode;
        lastNode = curNode;
    }
    int val, pos;
    char buf[100];
    while (~scanf("%s", buf)) {
        scanf("%d", &pos);
        if (strcmp(buf, "@") == 0) {
            update(headNode);
        }
        else if (strcmp(buf, "#") == 0) {
            change(headNode, pos);
            update_partial(headNode, pos);
        } 
        else {
            val = parseInt(buf);
            insert(headNode, pos, val);
            update_partial(headNode, pos);
        }
        if (headNode->next == NULL) {
            puts("You win!");
            break;
        } else {
            printList(headNode);
            putchar('\n');
        }
    }
}